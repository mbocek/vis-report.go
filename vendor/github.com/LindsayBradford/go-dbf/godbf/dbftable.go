...
func (dt *DbfTable) AddDateTimeField(fieldName string) (err error) {
	return dt.addField(fieldName, DateTime, Date.fixedFieldLength(), DateTime.decimalCountNotApplicable())
}
...
func (dt *DbfTable) FieldValue(row int, fieldIndex int) (value string) {
	...
	if dt.fields[fieldIndex].fieldType == DateTime {
		dateTime, err := dt.parseDateTime(temp)
		if err != nil {
			panic(err)
		}
		value = dateTime.Format("20060201150405")
	} else {
...
func (dt *DbfTable) parseDateTime(raw []byte) (time.Time, error) {
	if len(raw) != 8 {
		return time.Time{}, errors.New("invalid field pos")
	}
...
func (dt *DbfTable) GetRowAsMap(row int) map[string]string {

	s := make(map[string]string)
	for i := 0; i < len(dt.Fields()); i++ {
		fieldName := dt.Fields()[i].name
		s[fieldName] = dt.FieldValue(row, i)
	}
